...
def drawMarker(screen, screen_pick):
    innerRadius = 5
    middleRadius = innerRadius + 1
    outerRadius = middleRadius + 1
    lineWidth = 2

    innerRect = (screen_pick[0]-innerRadius, screen_pick[1]-innerRadius, \
                 2*innerRadius, 2*innerRadius)
    middleRect = (screen_pick[0]-middleRadius, screen_pick[1]-middleRadius, \
                 2*middleRadius, 2*middleRadius)
    outerRect = (screen_pick[0]-outerRadius, screen_pick[1]-outerRadius, \
...
by right-clicking.
    ...
    # Decimation divisors.  See common_calib.
    dec_divisor_width = imgSize[0] / common_calib.output_width
    dec_divisor_height = imgSize[1] / common_calib.output_height
    ...
    while not done:
        ...
        for pick in pickList:
            screen_pick = (scaleFactor * dec_divisor_width * pick[0],
                           scaleFactor * dec_divisor_width * pick[1])
            drawMarker(screen, screen_pick)
        pygame.display.flip()
